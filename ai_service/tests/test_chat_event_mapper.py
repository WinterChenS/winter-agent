...
def test_process_stream_token_event_buffers_partial_control_json():
    ...
    rewritten, collecting, buffer = process_stream_token_event(event, False, "", {"search", "time"})
...
def test_process_stream_token_event_filters_tool_control_json():
    ...
    _, collecting, buffer = process_stream_token_event(part1, False, "", {"search", "time"})

    part2 = {"event": "on_chat_model_stream", "data": {"chunk": _Chunk('"tool": "search", "query": "x"}')}}
    rewritten, collecting, buffer = process_stream_token_event(part2, collecting, buffer, {"search", "time"})
