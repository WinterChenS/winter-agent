...
async def test_full_pipeline_discover_and_invoke():
    ...
    assert tool_names >= {"search", "time", "browser", "execute_python"}
    ...
    result = await registry.invoke("execute_python", {"code": "print(42)"})
    assert result["ok"] is True
    ...
    time_result, sandbox_result = await asyncio.gather(
        ...
        registry.invoke("execute_python", {"code": "print(99)"}),
    )
    ...
    # execute_python with empty code fails with INVALID_INPUT,
